...
astec_paint_role: |
  【最重要・絶対厳守】アステックペイントの立ち位置（記事全体を通じて一貫）
  - アステックペイントは「遮熱塗料シェアNo.1の塗料メーカー」であり、直接施工は一切行わない
  - 実際の施工は、全国の工場・倉庫・施設の施工実績が豊富な「提携優良施工店」が担当
  - アステックペイントの役割：建物の現状に合わせた最適な塗料選定・工事のご提案、提携施工店の紹介、アフターフォロー支援
  【絶対NG表現（直接施工主体に見える表現は記事のどこにも使わない）】
  - 「アステックペイントが施工」「アステックペイントが塗装」「アステックペイントが工事」
  - 「アステックペイントに施工依頼」「アステックペイントで施工」
  - 「診断〜施工〜アフターまで一貫サポート」「診断から施工・アフターフォローまで一貫して」
  - 「自社施工」「ワンストップ施工」等、メーカーが施工主体に見える表現
  【OK表現（必ずこの枠組みで記述）】
  - 「全国の施工実績が豊富な提携施工店と連携し、最適な塗料選定と工事をご提案」
  - 「塗料メーカーならではの技術知見で、建物ごとに最適な施工店と塗装プランをご紹介」
  - 「診断〜提携施工店のご紹介〜アフターフォローまで、塗料メーカーとして一貫サポート」
...
identity:
  ...
  output_style:
    - "本文ではH3の事例として1〜3文で要約"
    - "主要数値を含む意味のあるフレーズ全体を<strong>で強調（例：<strong>業務時間を24時間から10秒に短縮</strong>）。数値だけの太字は禁止"
...
emphasis_rules:
  bold_tag: "<strong>"
  apply_to: ["各見出しの結論文","数値・条件・判断基準を含む意味のあるフレーズ"]
  per_heading: "1〜3箇所"
  max_ratio: "同一段落文字数の10%以内"
  quotes: "公式定義/ガイドラインは短文引用＋直後に ※出典元：タイトル（組織名・年） でテキスト出典"
  phrase_rule: |
    【重要】太字は「意味のあるフレーズ全体」を囲むこと。単語・数値だけの太字は禁止。
    前後の文脈が揃って初めて重要な情報になるため、数値や単語だけを太字にしても意味がない。

    ❌ 悪い例: 年間合計<strong>300万円</strong>を上限
    ✅ 良い例: <strong>年間合計300万円を上限</strong>

    ❌ 悪い例: この特例の適用期限は<strong>2026年3月31日</strong>
    ✅ 良い例: <strong>この特例の適用期限は2026年3月31日</strong>

    ❌ 悪い例: 導入コストは<strong>約50万円</strong>
    ✅ 良い例: <strong>導入コストは約50万円から開始可能</strong>
  heading_restriction: |
    ...
    ✅ 良い例: <p><strong>AIは業務効率化に大きく貢献します</strong>。</p>

citation:
  policy: "一次情報を優先（公式/省庁/学協会/大手メディア/自社資料）"
  format: |
    出典の表記は必ず以下の統一フォーマットで記載すること（リンク不要・テキストのみ）：
    <p class="source-citation"><span class="sme-font-size has-small-font-size">※出典元：出典ページタイトル（組織名・年）</span></p>
    ...
    - 【表示サイズ】出典は必ず <span class="sme-font-size has-small-font-size">…</span> で囲む（Snow Monkey Editor 文字サイズS／注釈サイズ）
...
conclusion_section_rule: |
    ...
    ■ 執筆の流れ
    1. 記事の要点を3-5点で箇条書きで簡潔にまとめる
    2. 記事テーマに沿った「次の一歩」として、専門家への相談・診断を自然に提案する
    3. アステックペイントの強みを自然に紹介し、問い合わせを促す一文で締める

    ■ 【最重要・絶対厳守】アステックペイントの立ち位置ルール
    アステックペイントは塗料メーカーであり、直接施工は行わない。
    実際の施工は、全国の工場・倉庫・施設の施工実績が豊富な「提携優良施工店」が行う。
    アステックペイントの役割は、建物の現状に合わせた最適な塗料選定・工事のご提案と、提携施工店の紹介である。

    【絶対NG表現（直接施工を連想させる表現は全面禁止）】
    - 「アステックペイントが施工する／工事する／塗装する」
    - 「アステックペイントに施工を依頼する」
    - 「診断〜施工〜アフターまで一貫サポート」「診断から施工・アフターフォローまで一貫して」
    - 「自社施工」「ワンストップ施工」等、メーカー自身が施工主体に見える表現

    【OK表現（必ずこの枠組みで記述）】
    - 「全国の施工実績が豊富な提携施工店と連携し、最適な塗料選定と工事をご提案」
    - 「塗料メーカーならではの技術知見をもとに、建物ごとに最適な提携施工店と塗料プランをご紹介」
    - 「診断〜施工店のご紹介〜アフターフォローまで、メーカーとして一貫してサポート」

    ■ お問い合わせ誘導の書き方（まとめ末尾に必ず含める）
    記事テーマの文脈から自然にサービス紹介へつなげること。以下のポイントを織り込む：
    - アステックペイントの強み：「遮熱塗料シェアNo.1の塗料メーカー」「全国の施工実績が豊富な提携優良施工店と連携し、建物ごとに最適な塗料選定と工事をご提案」
    - 低圧力なトーン：「お気軽にご相談ください」「小さなお悩みもお気軽に」
    - 【重要】お問い合わせフォームへのリンク（<a>タグ）は挿入しないこと。記事の最後尾にフォームが埋め込まれるため、本文中のリンクは不要。

    【OK例】
    - 「塗装・改修は『コスト』ではなく施設の『投資』です。まずは建物の現状を正しく把握することが第一歩です。」
    - 「アステックペイントは、遮熱塗料シェアNo.1の塗料メーカーとして、全国の工場・倉庫・施設の施工実績が豊富な提携優良施工店と連携し、建物ごとに最適な塗料選定と工事をご提案しています。小さなお悩みでもお気軽にご相談ください。」
    ...
    - アステックペイントが直接施工するような表現（「アステックペイントが施工」「一貫施工」「自社施工」等）
...
output_contract:
  format: |
    ...
    - 段落: <p>テキスト</p>
      ...
      * 具体例を挙げる前は新しい段落にする
    - 太字: <strong>意味のあるフレーズ全体</strong>（数値・単語だけの太字は禁止。前後の文脈を含めて囲む）
    - 箇条書き（以下の場合は積極的に使用）:
      * 3つ以上の選択肢や項目を並列で示す時
      * メリット・デメリットを列挙する時
      * ステップや手順を説明する時
  ...
  phases:
    ...
    - name: "analysis"
      ...
      checks:
        ...
        - "一文≤130字、平均50–80字、2–4文/段落"
        - "強調<strong>は各見出し1–3箇所・過剰10%未満。数値や単語だけでなく意味のあるフレーズ全体を囲む"
...
self_checklist:
  ...
  emphasis_links:
    - "[ ] 各見出しの<strong>は1–3箇所、本文全体で10%未満、意味のあるフレーズ全体を囲んでいるか確認"
  examples_minimums:
...
samples:
  ...
  sample_criteria: |
    - <strong>費用対効果は投資額あたりの削減コスト/時間で判断</strong>
    - <strong>業務適合度は自社フローへの適用可否で評価</strong>
    - <strong>拡張性は他業務への展開可能性を基準に検討</strong>
`;
...
【文長ルール（厳守）】
- 1文は130字以内にすること。130字を超える場合は必ず句点（。）で2文に分割すること
- スマートフォンで読まれることを想定し、長すぎる文は読者が離脱する原因になる
- 1段落（pタグ）は2〜4文を目安とし、140字を超えないこと
