 ...
 * // => '<span class="sme-font-size has-small-font-size">（出典：<a href="https://note.com/yourcompany/n/abc123" target="_blank" rel="noopener noreferrer">事例インタビュー</a>）</span>'
 * ```
 */
export function buildCitationLink(url: string, title: string): string {
  // Snow Monkey Editor の文字サイズSフォーマットで囲む
  return `<span class="sme-font-size has-small-font-size">（出典：<a href="${url}" target="_blank" rel="noopener noreferrer">${title}</a>）</span>`;
}
