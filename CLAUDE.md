...
### 自社サービス訴求H2を落とさないための実装（絶対厳守）

自社サービス訴求は競合記事に存在しないH2のため、以下3点が揃っていないとモデルがH2上限に収めようとして省略する。**いずれも削除・簡略化してはならない。**

1. **H2/H3の予算に枠を確保** — `outlineGeneratorV2.ts` の `serviceH2Addition = 1` / `serviceH3Addition = 2` を競合平均に加算する。`OutlineDisplayV2.tsx` の「サービス訴求追加後: H2=平均+1 / H3=平均+2」表示と一致させること
2. **プロンプトの固定順序** — `固定順序: 最後3つ: ["FAQ（任意）", "自社サービス訴求（必須）", "まとめ（必須）"]`。「最後2つ」に戻すと訴求章が生成されなくなる
3. **バリデーション** — `outlineCheckerV2.ts` がまとめの直前に自社名を含むH2があるか、そのH3数が2〜3個かを検証する。欠落時は error を返し自動修正に回る

FAQは競合の30%以上が含む場合のみ生成される仕様（任意）のため、出力されないことがあるが正常。
