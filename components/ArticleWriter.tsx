...
import { generateArticleV3, fixWordPressListBlocks, fixWordPressTableBlocks } from "../services/writingAgentV3";
...
function cleanupArticleContent(content: string): string {
  ...
  // 3. WordPress ブロックエディタ互換のリストHTML整形（最終段階で必ず適用）
  cleaned = fixWordPressListBlocks(cleaned);

  // 4. WordPress ブロックエディタ互換のテーブルHTML整形
  cleaned = fixWordPressTableBlocks(cleaned);

  // 5. H2/H3 見出しに番号付与（H2: "1. ", H3: "1-1. "）— 冪等
  ...
  const handleGenerateArticle = async () => {
    ...
    try {
      ...
      if (writingMode === "v3") {
        ...
        // クリーンアップ処理を実行（V3は既にHTMLなのでMarkdown→HTML変換は不要）
